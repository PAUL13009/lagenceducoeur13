...
import { createEstimationRequest } from '@/lib/firestore';

export default function FormulaireEstimationEtape2Page() {
  ...
  useEffect(() => {
    const etape1 = localStorage.getItem('estimation_etape1');
    console.log('Données récupérées du localStorage:', etape1);
    if (etape1) {
      try {
        const parsed = JSON.parse(etape1);
      ...
      console.warn('Aucune donnée trouvée dans localStorage pour estimation_etape1');
  ...
  const validateForm = () => {
    ...
    // Si bien loué, loyer et fin de bail requis
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Bouton cliqué - Début de handleSubmit');
    console.log('etape1Data:', etape1Data);
    console.log('formData:', formData);
    
    // Vérifier etape1Data
    if (!etape1Data) {
      console.error('Erreur: etape1Data est null ou undefined');
      alert('Erreur: Les données de l\'étape 1 sont manquantes. Veuillez recommencer depuis le début.');
      return;
    }
    ...
    try {
      ...
      const requestId = await createEstimationRequest(completeData);
      console.log('Demande d\'estimation créée avec succès, ID:', requestId);

      // Nettoyer le localStorage
      localStorage.removeItem('estimation_etape1');
            ...
            <div className="max-w-4xl mx-auto">
              <form onSubmit={handleSubmit} className="space-y-12">
                ...
                <div className="bg-gray-50 p-8 rounded-lg border border-gray-200">
                  <div className="space-y-4 mb-6">
                    <p className="text-gray-700 leading-relaxed">
                      Les estimations réalisées par L'Agence du Cœur sont fondées sur une analyse précise du bien et du marché marseillais.
                    </p>
                    <p className="text-gray-700 leading-relaxed">
                  ...
                  <div>
                    <label className="flex items-start p-4 border-2 rounded-lg cursor-pointer hover:bg-white transition-colors" style={{ borderColor: formData.confirmation ? '#1a2332' : '#e5e7eb' }}>
                      ...
                      <span className="text-gray-700">
                        Je confirme que mon projet de vente est réel et que je souhaite une estimation professionnelle, basée sur la réalité du marché. <span className="text-red-500">*</span>
                      </span>
                    </label>
                ...
                <div className="text-center space-y-4">
                  {/* Afficher un message si etape1Data est manquant */}
                  {!etape1Data && (
                    <div className="bg-red-50 border-2 border-red-200 rounded-lg p-4 mb-4">
                      <p className="text-red-700 font-semibold">⚠️ Erreur : Les données de l'étape 1 sont manquantes.</p>
                      <p className="text-red-600 text-sm mt-2">Veuillez recommencer depuis le début du formulaire.</p>
                      <Link 
                  ...
                  {Object.keys(errors).length > 0 && (
                    <div className="bg-yellow-50 border-2 border-yellow-200 rounded-lg p-4 mb-4">
                      <p className="text-yellow-800 font-semibold mb-2">⚠️ Veuillez corriger les erreurs suivantes :</p>
                  ...
                  >
                    <span>{loading ? 'Envoi en cours...' : 'Demander une estimation professionnelle'}</span>
                    {!loading && (
                      <svg 
