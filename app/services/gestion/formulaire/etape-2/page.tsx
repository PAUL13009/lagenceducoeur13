...
import { createGestionRequest } from '@/lib/firestore';

export default function FormulaireGestionEtape2Page() {
  ...
  const [formData, setFormData] = useState({
    ...
    // 3. Le bien est-il actuellement loué ?
    bienLoue: '',
    loyerActuel: '',
    typeBail: '',
    finBail: '',
    
    // 4. Montant approximatif des charges
    charges: '',
    taxeFonciere: '',
    loyersImpayes: '',
    
    // 5. Attentes spécifiques
    attentesSpecifiques: '',
  ...
  const bienLoueOptions = [
    { value: 'oui', label: 'Oui' },
    { value: 'non', label: 'Non' },
  ];

  const typeBailOptions = [
    { value: 'vide', label: 'Vide' },
    { value: 'meuble', label: 'Meublé' },
  ];

  const loyersImpayesOptions = [
    { value: 'oui', label: 'Oui' },
    { value: 'non', label: 'Non' },
  ];
  ...
  useEffect(() => {
    ...
    const storedData = localStorage.getItem('gestion_etape1');
    if (storedData) {
      try {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (type === 'checkbox') {
      ...
      if (name === 'equipements') {
        setFormData(prev => {
          const equipements = [...prev.equipements];
          if (checked) {
            if (!equipements.includes(checkboxValue)) {
              equipements.push(checkboxValue);
            }
          } else {
            const index = equipements.indexOf(checkboxValue);
            if (index > -1) {
              equipements.splice(index, 1);
            }
          }
          return { ...prev, equipements };
        });
      } else {
  ...
  const validateForm = () => {
    ...
    if (!formData.surfaceHabitable.trim()) {
      ...
      newErrors.surfaceHabitable = 'La surface doit être un nombre positif';
    }
    if (!formData.nombrePieces.trim()) {
      newErrors.nombrePieces = 'Le nombre de pièces est requis';
    } else if (isNaN(Number(formData.nombrePieces)) || Number(formData.nombrePieces) <= 0) {
      newErrors.nombrePieces = 'Le nombre de pièces doit être un nombre positif';
    }
    if (!formData.nombreChambres.trim()) {
      ...
      newErrors.nombreChambres = 'Le nombre de chambres doit être un nombre positif ou zéro';
    }
    ...
    if (!formData.bienLoue) newErrors.bienLoue = 'Cette information est requise';
    
    // Validation conditionnelle si le bien est loué
    if (formData.bienLoue === 'oui') {
      ...
      if (!formData.typeBail) newErrors.typeBail = 'Le type de bail est requis si le bien est loué';
    }
    
    if (!formData.loyersImpayes) newErrors.loyersImpayes = 'Cette information est requise';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const completeData = {
        ...
        pourquoi_gestion: etape1Data.pourquoiGestion,
        gestion_actuelle: etape1Data.gestionActuelle,
        objectif_principal: etape1Data.objectifPrincipal,
        niveau_implication: etape1Data.niveauImplication,
        cadre_clair: etape1Data.cadreClair,
      ...
      console.log('Demande de gestion créée avec succès, ID:', requestId);
              ...
              >
                ...
                <div className="bg-white p-8 rounded-lg shadow-md border border-gray-200">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        <label htmlFor="nombreEtages" className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    {/* Ascenseur */}
                    <div>
                      <label htmlFor="ascenseur" className="block text-sm font-medium text-gray-700 mb-2">
                ...
                <div className="bg-white p-8 rounded-lg shadow-md border border-gray-200">
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {etatsGeneraux.map((option) => (
                          <label key={option.value} className="flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors" style={{ borderColor: formData.etatGeneral === option.value ? '#1a2332' : '#e5e7eb' }}>
                            <input
                              type="radio"
                              name="etatGeneral"
                              value={option.value}
                              checked={formData.etatGeneral === option.value}
                              onChange={handleChange}
                              className="mr-4"
                            />
                            <span>{option.label}</span>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {equipementsOptions.map((option) => (
                          <label key={option.value} className="flex items-center p-3 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors" style={{ borderColor: formData.equipements.includes(option.value) ? '#1a2332' : '#e5e7eb' }}>
                            <input
                              type="checkbox"
                              name="equipements"
                              value={option.value}
                              checked={formData.equipements.includes(option.value)}
                              onChange={handleChange}
                              className="mr-3"
                            />
                            <span>{option.label}</span>
                  ...
                  <div className="space-y-6">
                    <div>
                      <div className="space-y-3 mb-6">
                        {bienLoueOptions.map((option) => (
                          <label key={option.value} className="flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors" style={{ borderColor: formData.bienLoue === option.value ? '#1a2332' : '#e5e7eb' }}>
                            <input
                              type="radio"
                              name="bienLoue"
                              value={option.value}
                              checked={formData.bienLoue === option.value}
                              onChange={handleChange}
                              className="mr-4"
                            />
                            <span>{option.label}</span>
                      ...
                      {errors.bienLoue && <p className="mt-2 text-sm text-red-500">{errors.bienLoue}</p>}
                    </div>

                    {/* Champs conditionnels si le bien est loué */}
                    {formData.bienLoue === 'oui' && (
                      <div className="space-y-6 pt-4 border-t border-gray-200">
                        ...
                        <div>
                          <label htmlFor="typeBail" className="block text-sm font-medium text-gray-700 mb-2">
                            Type de bail <span className="text-red-500">*</span>
                          </label>
                          <div className="space-y-3">
                            {typeBailOptions.map((option) => (
                              <label key={option.value} className="flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors" style={{ borderColor: formData.typeBail === option.value ? '#1a2332' : '#e5e7eb' }}>
                                <input
                                  type="radio"
                                  name="typeBail"
                                  value={option.value}
                                  checked={formData.typeBail === option.value}
                                  onChange={handleChange}
                                  className="mr-4"
                                />
                                <span>{option.label}</span>
                          ...
                          {errors.typeBail && <p className="mt-2 text-sm text-red-500">{errors.typeBail}</p>}
                        </div>
                ...
                <div className="bg-white p-8 rounded-lg shadow-md border border-gray-200">
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Loyers impayés en cours ? <span className="text-red-500">*</span>
                      </label>
                      <div className="space-y-3">
                        {loyersImpayesOptions.map((option) => (
                          <label key={option.value} className="flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors" style={{ borderColor: formData.loyersImpayes === option.value ? '#1a2332' : '#e5e7eb' }}>
                            <input
                              type="radio"
                              name="loyersImpayes"
                              value={option.value}
                              checked={formData.loyersImpayes === option.value}
                              onChange={handleChange}
                              className="mr-4"
                            />
                            <span>{option.label}</span>
                      ...
                      {errors.loyersImpayes && <p className="mt-2 text-sm text-red-500">{errors.loyersImpayes}</p>}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-8 rounded-lg shadow-md border border-gray-200">
                  ...
                  <div>
                    <label htmlFor="attentesSpecifiques" className="block text-sm font-medium text-gray-700 mb-2">
                      Avez-vous des attentes particulières concernant la gestion de votre bien ?
                    </label>
                    <textarea
                      id="attentesSpecifiques"
                      ...
                      className="w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all resize-none"
                      style={{ borderColor: '#1a2332' }}
                      placeholder="Précisez vos attentes particulières concernant la gestion de votre bien..."
                    />
                  </div>
                </div>

                {/* 6. Message de pré-validation */}
                <div className="bg-white p-8 rounded-lg shadow-md border border-gray-200">
                  <h2 className="text-2xl md:text-3xl font-normal mb-6 uppercase tracking-wider" style={{ fontFamily: "'Playfair Display', serif", letterSpacing: '0.08em', color: '#1a2332' }}>
                    6. Message de pré-validation
                  </h2>
                  ...
                  <div className="space-y-4 mb-6">
                    <p className="text-gray-700">
                      Les demandes de gestion locative sont étudiées manuellement.
                    </p>
                    <p className="text-gray-700">
                      Nous acceptons uniquement les projets compatibles avec une gestion rigoureuse, transparente et durable.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <span className="text-gray-700 cursor-pointer">
                        Je confirme souhaiter confier la gestion de mon bien dans un cadre professionnel et structuré. <span className="text-red-500">*</span>
                      </span>
                    </label>
                ...
                <div className="text-center space-y-4">
                  ...
                  >
                    <span>{loading ? 'Envoi en cours...' : 'Envoyer ma demande de gestion locative'}</span>
                    {!loading && (
                      <svg 
                  ...
                  <p className="text-sm text-gray-500">
                    Réponse réservée aux projets correspondant à notre méthode.
                  </p>
                </div>
              </form>
            </div>
